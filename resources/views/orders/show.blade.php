            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-4 transition-colors">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs text-slate-700 dark:text-slate-300">
                        <thead class="bg-slate-50 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 uppercase font-bold text-[10px]">
                            <tr>
                            ...
                            @foreach($order->orderItems as $item)
                                <tr>
                                    <td class="px-4 py-3 font-bold text-slate-900 dark:text-white">
                                        {{ $item->product->name ?? 'Deleted Product Item' }}
                                        <div class="text-[10px] text-slate-400 font-normal">
                                            Variant: {{ $item->product->size ?? 'N/A' }} • Color: {{ $item->product->color ?? 'Standard' }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-center font-bold text-slate-900 dark:text-white font-mono">{{ $item->quantity }}</td>
                                    <td class="px-4 py-3 text-right font-mono">₦{{ number_format($item->unit_price, 2) }}</td>
                                    <td class="px-4 py-3 text-right font-bold text-slate-900 dark:text-white font-mono">₦{{ number_format($item->subtotal, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between font-black text-slate-900 dark:text-white">
                    <span>Grand Total:</span>
                    <span class="text-xl text-amber-600 dark:text-amber-400 font-mono">₦{{ number_format($order->total_amount, 2) }}</span>
                </div>
            </div>
                ...
                <div class="space-y-3 text-xs">
                    ...
                    <div>
                        ...
                        <span class="font-bold text-slate-900 dark:text-white text-xs inline-flex items-center px-2.5 py-1 bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300 rounded-lg mt-0.5 border border-amber-200 dark:border-amber-900">
                            @if(($order->payment_method ?? 'cash') === 'cash') 💵 Cash Payment
                            @elseif(($order->payment_method ?? '') === 'bank_transfer') 💳 Bank Transfer
                            @elseif(($order->payment_method ?? '') === 'pos') 📱 POS / Card Machine
                    ...
                    @if(!empty($order->payment_proof))
                        <div class="pt-2 border-t border-slate-100 dark:border-slate-800">
                            ...
                            @else
                                <a href="{{ asset($order->payment_proof) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-slate-900 dark:bg-slate-800 text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition-all">
                                    📄 View Document Proof &rarr;
                                </a>
                            @endif
                        </div>
                    @endif
